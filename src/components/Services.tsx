...
export default function Services({ language, onBookDepartment }: ServicesProps) {
  ...
  return (
    <section id="services-section" className="py-16 sm:py-24 bg-slate-50/40 border-t border-slate-200 overflow-hidden">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <motion.p 
            ...
            className="mt-4 text-base text-slate-550 font-normal max-w-2xl mx-auto font-sans"
          >
            {t.servicesSubtitle}
          </motion.p>
          ...
          {DEPARTMENTS.map((dept, index) => {
            ...
            return (
              <motion.div 
                ...
                className="flex flex-col justify-between rounded-2xl border border-slate-200 bg-white p-6 shadow-xs hover:border-blue-400 transition-all duration-300 cursor-pointer"
              >
                <div>
                  {/* Icon */}
                  <motion.div 
                    whileHover={{ rotate: 15 }}
                    className="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-50 text-blue-600 mb-6 border border-blue-100"
                  >
        ...
        {/* Department Details Modal with Beautiful Smooth Slide Up Pops */}
        <AnimatePresence>
          {selectedDept && (
            <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
              <div className="flex min-h-screen items-end justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <motion.div 
                  ...
                  className="relative inline-block transform overflow-hidden rounded-2xl bg-white text-left align-bottom shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-2xl sm:align-middle border border-slate-200"
                >
                  {/* Close Button */}
                  <button
                    onClick={() => setSelectedDept(null)}
                    className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 transition focus:outline-hidden p-1.5 rounded-lg hover:bg-slate-50 cursor-pointer z-10"
                  >
                    <X className="h-5 w-5" />
                  </button>

                  {/* Modal Header Cover */}
                  <div className="bg-slate-900 p-6 sm:p-8 text-white border-b border-slate-800 relative overflow-hidden">
                    <div className="absolute top-0 right-0 h-24 w-24 bg-blue-500/10 rounded-full -mr-6 -mt-6" />
                    <div className="flex items-center gap-3 mb-2">
                      {React.createElement(getIcon(selectedDept.icon), { className: "h-7 w-7 text-blue-400 animate-pulse" })}
                      ...
                      <div className="space-y-4">
                        {selectedDept.doctors.map((doc, idx) => (
                          <motion.div 
                            key={idx} 
                            whileHover={{ x: 4 }}
                            className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 rounded-xl border border-slate-200 bg-slate-50/40"
                          >
                            ...
                            <span className="mt-2 sm:mt-0 inline-flex items-center rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-medium text-blue-800 border border-blue-100 self-start sm:self-center">
                      ...
                      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {selectedDept.features[language].map((feat, idx) => (
                          <motion.li 
                            key={idx} 
                            whileHover={{ x: 3 }}
                            className="flex items-start gap-2 text-sm text-slate-600 leading-normal"
                          >
                  ...
                  <div className="bg-slate-50 px-6 py-4 sm:px-8 sm:py-5 flex flex-col sm:flex-row sm:justify-between items-center gap-3 border-t border-slate-200">
                    ...
                    <div className="flex gap-3 w-full sm:w-auto">
                      ...
                      <motion.button
                        ...
                        className="w-full sm:w-auto px-5 py-2 text-sm font-bold text-white bg-blue-600 rounded-xl hover:bg-blue-700 cursor-pointer text-center flex items-center justify-center gap-1.5 shadow-sm"
                      >
                        <Calendar className="h-4 w-4 text-blue-200 animate-bounce" />
